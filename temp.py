...
def load_known_faces():
    records = main.load_known_faces_from_database()
    ...
    for filename in os.listdir(known_faces_dir):
        if filename.endswith(".jpg") or filename.endswith(".png"):
            ...
            if any(np.all(encoding == known_face_encoding) for known_face_encoding in known_face_encodings):
                continue
            main.storing_face_encodings(first_name, last_name, encoding)
...
while True:
    ...
    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
        ...
        if min_distance <= confidence_threshold: #and is_real_face :
            ...
            print(data)
            if (first_name != temp_name and last_name != temp_surname): main.record_arrival(first_name, last_name)
